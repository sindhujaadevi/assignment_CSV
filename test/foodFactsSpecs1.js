...
const convert = require('../js/foodFactsPart1');
let jsonDiff = require('./jsondiff');
let totalObjectKeys = require('./totalObjectKeys');
let expectedJSON = require('./expectedjson/expectedJsonPart1.json');
let actualJSON = require('../outputdata/stackedBar.json');
...
describe('Test Application as WhiteBox', function() {
    ...
    it('should fail if the input is not array', function(done) {
    ...
    it('should output the input that we send with correct index', function(done) {
        let input = ['United Kingdom', 'Denmark', 'Sweden', 'Norway'];
        let result = convert.main(input);
        result.should.be.equal(input).and.have.lengthOf(input.length);
      //  console.log(result);
        done();
    });
    it('should return country index', function(done) {
        let input = 'Canada';
